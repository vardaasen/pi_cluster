...
start_cluster() {
    local alias_var
    local alias
    log_info "Starting Kafka cluster on all $NODE_COUNT nodes..."

    for (( i=1; i<=NODE_COUNT; i++ )); do
        alias_var="NODE_${i}_ALIAS"
        alias="${!alias_var}"
        log_info "  -> Starting Kafka on $alias"
        ssh "$alias" 'docker compose up -d' &
    done

    wait
    log_info "Cluster startup initiated on all nodes."
}
start_cluster
